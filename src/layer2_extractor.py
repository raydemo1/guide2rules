...
def _build_system_prompt_for_extraction() -> str:
    return (
        ...
        "regex 必须使用全量匹配（使用^和$），仅在唯一可识别且格式明确的字段上提供；禁止生成过于宽泛的表达式，如可匹配任意11–13位数字的通用数字表达式；金额字段必须强制两位小数格式（如 ^(0|[1-9]\\d*)\\.\\d{2}$），"
        "仅对常见的强规则字段给出正则，例如身份证号、手机号、邮箱、统一社会信用代码、银行卡号、IPv4/IPv6等；不得给非结构化字段或宽泛字段生成正则。关键词也必须在不同item间不得重复。"
